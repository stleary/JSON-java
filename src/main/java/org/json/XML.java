...
public class XML {
            ...
            for (; ; ) {
                ...
                if (token instanceof String) {
                    ...
                    if (token == EQ) {
                        ...
                        } else if (config.getXsiTypeMap() != null
                                && !config.getXsiTypeMap().isEmpty()
                            ...
                            if (parse(x, jsonObject, tagName, config, currentNestingDepth + 1)) {
                                if (config.getForceList().contains(tagName)) {
                                    ...
                                    if (jsonObject.length() == 0) {
                                        ...
                                        context.append(
                                                tagName, jsonObject.opt(config.getcDataTagName()));
                                    } else {
                                        context.append(tagName, jsonObject);
                                    }
                                } else {
     ...
     * are ignored.
     *
     * <p>All values are converted as strings, for 1, 01, 29.0 will not be coerced to numbers but
     * will instead be the exact value as seen in the XML document.
     *
     * @param reader The XML source reader.
     * @param keepStrings If true, then values will not be coerced into boolean or numeric values
    ...
    public static String toString(
            final Object object, final String tagName, final XMLParserConfiguration config)
            throws JSONException {
        return toString(object, tagName, config, 0, 0);
    }

    /**
     * Convert a JSONObject into a well-formed, element-normal XML string, either pretty print or
     * single-lined depending on indent factor.
     *
     * @param object A JSONObject.
     * @param tagName The optional name of the enclosing tag.
     * @param config Configuration that can control output to XML.
