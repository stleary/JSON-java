 ...
 *
 * <p>At its most basic, a cookie is a name=value pair. The value may be subdivided into other
 * cookies, but that is not tested here. The cookie may also include certain named attributes,
 * delimited by semicolons.
 *
 * <p>The Cookie.toString() method emits certain attributes if present: expires, domain, path,
 * secure. All but secure are name-value pairs. Other attributes are not included in the toString()
 * output.
 *
 * <p>A JSON-Java encoded cookie escapes '+', '%', '=', ';' with %hh values.
 */
public class CookieTest {

    /** Attempts to create a JSONObject from a null string. Expects a NullPointerException. */
    ...
    /**
     * Attempts to create a JSONObject from a cookie string with no '=' char. Expects a
     * JSONException.
     */
    @Test
    ...
    /**
     * Attempts to create a JSONObject from a cookie string with embedded ';' char. Expects a
     * JSONException.
     */
    @Test
    ...
    public void emptyStringCookieException() {
        ...
        try {
            ...
            assertEquals(
                    "Expecting an exception message", "Cookies must have a 'name'", e.getMessage());
        }
    }
    /**
    ...
    /**
     * Store a cookie with all of the supported attributes in a JSONObject. The secure attribute,
     * which has no value, is treated as a boolean.
     */
    @Test
    ...
    /**
     * Cookie.toString() will emit the non-standard "thiswont=beIncluded" attribute, and the
     * attribute is still stored in the JSONObject. This test confirms both behaviors.
     */
    @Test
    public void convertCookieToString() {
        ...
        JSONObject jsonObject = Cookie.toJSONObject(cookieStr);
        JSONObject expectedJsonObject = new JSONObject(expectedCookieStr);
        JSONObject expectedDirectCompareJsonObject = new JSONObject(expectedDirectCompareCookieStr);
        // emit the string
    ...
    /**
     * A string may be URL-encoded when converting to JSONObject. If found, '+' is converted to ' ',
     * and %hh hex strings are converted to their ascii char equivalents. This test confirms the
     * decoding behavior.
     */
    @Test
    ...
    /**
     * A public API method performs a URL encoding for selected chars in a string. Control chars,
     * '+', '%', '=', ';' are all encoded as %hh hex strings. The string is also trimmed. This test
    ...
    public void escapeString() {
        ...
        assertTrue(
                "expect escape() to encode correctly. Actual: "
                        + actualStr
                        + " expected: "
                        + expectedStr,
    ...
    /**
     * A public API method performs URL decoding for strings. '+' is converted to space and %hh hex
     * strings are converted to their ascii equivalent values. The string is not trimmed. This test
     * confirms that behavior.
     */
    @Test
    public void unescapeString() {
        ...
        assertTrue(
                "expect unescape() to decode correctly. Actual: "
                        + actualStr
                        + " expected: "
                        + expectedStr,
                expectedStr.equals(actualStr));
