...
//    }
    ...
    public void testToJSONArrayMaxNestingDepthOf42IsRespected() {
        ...
        try {
            JSONML.toJSONArray(wayTooLongMalformedXML, JSONMLParserConfiguration.getOriginalConfiguration().withMaxNestingDepth(maxNestingDepth));
    ...
    public void testToJSONArrayMaxNestingDepthIsRespectedWithValidXML() {
        ...
        try {
            JSONML.toJSONArray(perfectlyFineXML, JSONMLParserConfiguration.getOriginalConfiguration().withMaxNestingDepth(maxNestingDepth));
    ...
    public void testToJSONObjectMaxDefaultNestingDepthIsRespected() {
        ...
        try {
            JSONML.toJSONObject(wayTooLongMalformedXML, JSONMLParserConfiguration.getOriginalConfiguration());

            fail("Expecting a JSONException");
        } catch (JSONException e) {
            assertTrue("Wrong throwable thrown: not expecting message <" + e.getMessage() + ">",
                e.getMessage().startsWith("Maximum nesting depth of " + JSONMLParserConfiguration.getDefaultMaximumNestingDepth()));
        }
    }

    @Test
    public void testToJSONObjectUnlimitedNestingDepthIsPossible() {
        int actualDepth = JSONMLParserConfiguration.getDefaultMaximumNestingDepth() +10;
        final String deeperThanDefaultMax = new String(new char[actualDepth]).replace("\0", "<a>") +
            "value" +
            new String(new char[actualDepth]).replace("\0", "</a>");

        try {
            JSONML.toJSONObject(deeperThanDefaultMax, JSONMLParserConfiguration.getOriginalConfiguration()
    ...
    public void testToJSONObjectMaxNestingDepthOf42IsRespected() {
        ...
        try {
            JSONML.toJSONObject(wayTooLongMalformedXML, JSONMLParserConfiguration.getOriginalConfiguration().withMaxNestingDepth(maxNestingDepth));
    ...
    public void testToJSONObjectMaxNestingDepthIsRespectedWithValidXML() {
        ...
        try {
            JSONML.toJSONObject(perfectlyFineXML, JSONMLParserConfiguration.getOriginalConfiguration().withMaxNestingDepth(maxNestingDepth));
